...
// --- Menu Drawing ---
void drawMotorSpeedMenu() {
    display.clearDisplay();
  
    // Background tetap
    display.drawBitmap(0, 0, epd_bitmap_gg, 128, 64, WHITE);
  
    display.setTextSize(1);
    display.setTextColor(WHITE);
    display.setCursor(64 - 32, 2);
    display.print("MOTOR SPEED");
  
    if (editingParam) {
      display.setCursor(64 - 3, 64 - 7); // Sedikit lebih ke bawah biar rata
      display.print("[E]");
    }
  
    // Halaman ditentukan berdasarkan indeks
    int baseIdx = (selectedParamIndex < 2) ? 0 : 2;
  
    const char* motorLabels[4] = {"M FL", "M FR", "M BL", "M BR"};
    int posX[2] = {20, 88};    // Kolom kiri dan kanan
    int speedY = 20;           // Y untuk angka motorSpeed
    int labelY = 36;           // Y untuk label
  
    for (int i = 0; i < 2; i++) {
      int idx = baseIdx + i;
      bool isSelected = (idx == selectedParamIndex) && editingParam;
  
      // Highlight angka saja
        if (isSelected) {
        display.fillRect(posX[i] - 2, speedY - 2, 28, 12, WHITE);
        display.setTextColor(BLACK);
        display.setCursor(posX[i], speedY);
        display.print(motorSpeed[idx]);
        display.setTextColor(WHITE);  // Ganti warna kembali untuk label
        } else {
        display.setTextColor(WHITE);
        display.setCursor(posX[i], speedY);
        display.print(motorSpeed[idx]);
        ...
        // Label motor tetap putih
        display.setCursor(posX[i], labelY);
        display.print(motorLabels[idx]);
    }
  ...
  void drawStepperMenu() {
    display.clearDisplay();
    display.setCursor(0, 0);
    display.print("Mode: STEP LIFTER");
    ...
    for (int i = 0; i < 4; i++) {
      ...
      display.print(i);
      display.print(": ");
      display.println(stepPos[i]);
    }
    display.display();
  }
  
  void drawMotorLifterMenu() {
    display.clearDisplay();
    display.setCursor(0, 0);
    display.print("Mode: MOT LIFTER");
    if (editingParam) display.print(" [E]");
    display.println();
    ...
    for (int i = 0; i < 5; i++) {
      display.print(i == selectedParamIndex ? "> " : "  ");
      if (i < 4)
        display.println(String("LPos") + i + ": " + motLifterPos[i]);
      else
        display.println(String("LSpeed: ") + speedLifter);
    }
    display.display();
  }
  ...
  void drawGripMenu() {
    display.clearDisplay();
    display.setCursor(0, 0);
    display.print("Mode: GRIP");
    if (editingParam) display.print(" [E]");
    display.println();
  
    const char* labels[] = {"G1", "G2", "OpenG1", "OpenG2", "Use2"};
    int values[] = {grip1, grip2, openG1, openG2};
    ...
    for (int i = 0; i < 5; i++) {
      ...
      display.print(": ");
      if (i == 4)
        display.println(useGrip2 ? "YES" : "NO");
      else
  ...
  void drawSystemMenu() {
    display.clearDisplay();
    display.setCursor(0, 0);
    display.print("Mode: SYSTEM");
    if (editingParam) display.print(" [E]");
    display.println();
  
    display.print(selectedParamIndex == 0 ? "> " : "  ");
  ...
  void drawModeMenu() {
    switch (selectedMode) {
      case 0: drawMotorSpeedMenu(); break;
      case 1: drawStepperMenu(); break;
      case 2: drawMotorLifterMenu(); break;
      case 3: drawGripMenu(); break;
